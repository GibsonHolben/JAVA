...
public class MyPannel extends JPanel
{
	...
	public void DrawMiddleNumber()
	{
		...
		{
			g2D.setFont(new Font("URW Grotesk", 
							 	 Font.BOLD,
								 100));
			g2D.drawString(PlayerDeckText, 
						   1537, 
						   857);
			g2D.setPaint(Color.white);
			g2D.drawString(PlayerDeckText, 
	...
	public void PaintCard(String Type, int x, int y, int w, int h, Color Color)
	{
		...
		if(Type.equals("Main"))
		{
			if(MainDeckText.equals("W"))
			{
				//Exists so the card does not get drawn
			}
			...
			{
				//Draws the text for a normal card
				g2D.setPaint(Color.black);
